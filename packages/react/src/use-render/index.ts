...
// Replace when base-ui useRender is published as a package.
...
function getStateAttributes<TState extends RenderState>(
  ...
  for (const key of Object.keys(state) as (keyof TState)[]) {
    ...
    const attribute = `data-${String(key).replaceAll(
      /[A-Z]/gu,
...
>({
  ...
  // Required for the render-prop element merge path (base-ui useRender shape).
